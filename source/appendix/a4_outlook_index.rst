...
各章 ``outlook.rst`` 采用统一结构：工业界现状 → 值得关注的变化 → 与本教程衔接 → 进一步了解。贡献者更新时请保持简短，并标注大致时效（如「约 2025–2026 年」）。

各章 ``references.rst`` 提供本章内容索引（章节内 :doc: 链接）、示例代码路径、手册页与相关章节链接。更新正文或 lab 时请同步检查 ``references.rst``。

详见根目录 ``CONTRIBUTING.rst``。

章节文档结构
==========================

每章推荐阅读顺序：原理小节 → 实践 lab → outlook 延伸与趋势 → references 参考资料（各章均有同名文件）。
