...
传统上 root 拥有全部权力。:strong:`Capabilities` 将 root 特权拆分为独立单元，进程可只保留所需部分，实现最小权限。本节介绍 capability 集合、常用 capability 和 setcap 工具。

能力位图
========================

Linux 将特权分为约 40 个:strong:`capability` ，每个为一位。进程的 capability 分集合管理：
...
可给可执行文件设置:strong:`文件 capability` ，运行该程序时自动获得指定能力，无需完整 root：
...
子进程默认:strong:`继承` 父进程的 capability 集合。``execve`` 加载新程序时，根据文件 capability 位和可执行文件属性重新计算：
...
.. list-table::
   ...
   * - ``ei`` （effective+inheritable）
     - 需配合进程 inheritable 集
   * - ``p`` （permitted only）
     - 进入 permitted，需手动启用 effective

``setcap cap_net_bind_service+ep /usr/bin/myserver`` 后，任何用户运行 ``myserver`` 均可绑定 80 端口，无需 root。但若程序有漏洞，攻击者直接获得该能力——应配合最小权限代码审计。

Docker capability 映射
========================
...
``--privileged`` 等价于赋予几乎全部 capability 并放宽设备访问，生产环境应避免。``capsh --decode=$(grep CapEff /proc/self/status | awk '{print $2}')`` 可解码当前 effective 位图。
