...
#. 在 Ubuntu 上 ``aa-status``，在 RHEL 系上 ``getenforce``，记录本机默认 MAC 方案。
#. 对测试目录 ``setfacl -m u:otheruser:rwx shared``，用另一用户验证 ACL 覆盖 DAC 限制的场景。
#. 运行 ``docker run --rm --cap-drop ALL alpine capsh --print``，列出容器内剩余 capability 位。
