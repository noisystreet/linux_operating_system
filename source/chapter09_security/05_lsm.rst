...
Landlock 自 sandbox 示例（概念）
========================

Landlock 规则由进程自愿施加于自身及后续子进程，适合不可信插件、沙箱运行时：

.. code-block:: cpp

   #include <linux/landlock.h>
   #include <sys/syscall.h>
   // 使用 landlock_create_ruleset / landlock_restrict_self
   // 限制进程只能访问指定目录下的文件

规则可限定：读/写/执行/递归访问等。与 seccomp 互补——seccomp 限制"能调用哪些 syscall"，Landlock 限制"即使 syscall 允许，能触及哪些路径"。

BPF LSM 简述
========================

``BPF_PROG_TYPE_LSM`` 程序挂载到 LSM 钩子（如 ``file_open``、``socket_connect``），在内核执行 MAC 决策。云厂商可用其实现自定义策略引擎，无需编写完整内核模块或维护 SELinux 策略包。加载仍受 eBPF 验证器约束，比可随意 ``insmod`` 的模块更安全。

堆叠时的拒绝语义
========================

多个 LSM 同时启用时，:strong:`任一模块拒绝即拒绝` （stacking 默认策略）。``/sys/kernel/security/lsm`` 列出加载顺序。排错时需分别查看 SELinux AVC 日志、AppArmor ``dmesg`` 输出、Landlock 相关错误码。
