...
进程是资源分配的单元，但切换开销大。:strong:`线程` （thread）在同一进程内共享地址空间和资源，是 CPU 调度的基本单位。本节介绍用户级与内核级线程、Linux 的 NPTL 实现，以及线程与进程的关系。
...
早期 Linux 使用:strong:`LinuxThreads` ，每个线程对应一个轻量级进程，通过 ``clone()`` 实现，但不符合 POSIX 语义（如信号处理）。2003 年:strong:`NPTL` （Native POSIX Threads Library）成为标准，线程与内核调度实体一一对应，完全兼容 POSIX 线程 API。
...
同一进程的所有线程共享:strong:`TGID` （Thread Group ID），在 ``ps`` 中显示为同一 PID。每个线程有独立的:strong:`TID` ，可通过 ``gettid()`` 获取（glibc 未直接暴露，需 ``syscall(SYS_gettid)``）。
...
进程启动时有一个:strong:`主线程` ，``main()`` 在其上运行。主线程退出且其他线程未 join 时，整个进程终止（可用 ``pthread_exit()`` 让主线程先结束，工作线程继续）。:strong:`分离线程` （detached）退出后自动回收，无需 join，适合"发后即忘"的任务。
