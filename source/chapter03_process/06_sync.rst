...
多进程或多线程访问共享资源时，若缺乏协调，会导致数据不一致、死锁等问题。**同步** （synchronization）机制确保同一时刻只有一个执行流访问临界区，或按约定顺序访问。本节介绍互斥锁、信号量、条件变量和 RCU。

临界区与竞态条件
========================

**临界区** （critical section）是访问共享资源的代码段。**竞态条件** （race condition）指多个执行流并发进入临界区，结果依赖于调度顺序，行为不可预测。

经典例子：两个线程各执行 100 万次 ``counter++``，期望结果为 200 万，实际往往小于 200 万——因为 ``counter++`` 非原子，涉及读-改-写三步，可能被交错执行。

解决思路：**互斥** （mutual exclusion）——同一时刻仅一个执行流在临界区内。

互斥锁（Mutex）
========================

**互斥锁** 是最基本的同步原语。加锁后，其他尝试加锁的线程阻塞，直到持有者解锁。
...
**信号量** 维护一个计数器，支持 ``wait`` （P 操作，减 1，若为 0 则阻塞）和 ``post`` （V 操作，加 1，唤醒等待者）。

- **二元信号量** ：取值为 0 或 1，等价于互斥锁
- **计数信号量** ：可用于资源池（如连接池最多 N 个连接）
...
**条件变量** 用于"等待某个条件成立"。线程在条件不满足时阻塞，其他线程改变条件后 ``signal`` 唤醒。
...
**死锁** 指多个执行流互相等待对方持有的锁，永远无法继续。四个必要条件（Coffman 条件）：
...
**读写锁** （rwlock）允许多个读者同时访问，写者独占。适合读多写少场景。
...
**RCU** （Read-Copy-Update）是 Linux 内核广泛使用的无锁同步机制，适合读多写极少场景（如路由表、链表遍历）。
...
Linux 内核 RCU 基于 **发布-订阅** 模型，核心 API 在 ``include/linux/rcupdate.h``：
...
读侧临界区在单核上禁止抢占，在多核上不可被调度到同 CPU 的 ``synchronize_rcu`` 阻塞，因此读者 **无锁** 且开销极低。

宽限期（Grace Period）
==========================

写者更新数据后，旧副本不能立即 ``kfree``——可能仍有读者持有旧指针。**宽限期** 是一段时间窗口，保证所有在更新前开始的读侧临界区都已结束：
...
- **路由表** ：``fib_rules``、``rtable`` 更新，读路径无锁遍历
- **模块链表** ：``module_list`` 的插入删除
- **PID 哈希** ：进程查找与 fork/exit 并发
- **文件系统 dentry** ：部分路径缓存更新
...
C++11 和 GCC 提供 **原子操作** ，硬件保证单条指令完成，无需锁：
...
Linux 的 pthread 互斥锁、条件变量底层依赖 **futex** （Fast Userspace Mutex）。无竞争时，加锁解锁完全在用户态完成；仅当需要阻塞/唤醒时才通过 ``futex()`` 系统调用进入内核。这使常见情况下的同步开销极低。
