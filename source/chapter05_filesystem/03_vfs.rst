...
Linux 同时支持 ext4、XFS、Btrfs、proc、sysfs 等数十种文件系统。若每种都暴露不同接口，应用程序将无法通用。**VFS** （Virtual File System）在应用程序与具体文件系统之间插入抽象层，提供统一的 open/read/write 接口。本节介绍 VFS 的核心对象和调用路径。
...
1. **统一接口** ：应用使用相同的系统调用访问任意文件系统
2. **可扩展** ：新文件系统只需实现 VFS 规定的操作集
3. **互操作** ：挂载在同一棵树上的不同 FS 对用户透明
...
**路径解析** （path lookup）从根 dentry 或当前工作目录出发，逐级查找。dentry 缓存（dcache）命中则跳过磁盘读取；未命中则从父目录 inode 读取目录项，创建新 dentry。

**挂载** 时，挂载点目录的 dentry 被 **遮盖** （covered），其下的 dentry 来自新挂载的 FS。``/home`` 挂载独立分区后，``/home/user`` 的解析进入该分区的根 inode。
...
块设备文件系统（ext4、XFS）通常通过 **page cache** 缓存数据，实际读写经过通用层；字符设备、proc 等直接实现 ``file_operations``。

文件描述符表
========================

进程通过 **文件描述符** （fd，0、1、2…）访问打开的文件。每个进程有 ``files_struct``，内含 fd 数组，指向内核的 ``struct file``。fork 后子进程复制 fd 表，父子可共享同一 file 的偏移（或独立，取决于 ``FD_CLOEXEC`` 等标志）。
...
VFS 维护 **dentry 缓存** 和 **inode 缓存** ，避免重复从磁盘读取元数据。内存紧张时，内核回收不常用的 dentry 和 inode。回收后再次访问需重新从磁盘加载。
...
**proc** 和 **sysfs** 不关联块设备，其 ``read/write`` 由内核函数动态生成内容。例如 ``cat /proc/cpuinfo`` 触发内核收集 CPU 信息并格式化为文本返回。

**tmpfs** 数据存于 page cache，无后端块设备，卸载或重启即丢失。适合 ``/tmp``、``/dev/shm``。

**pipe**、**socket** 也通过 VFS 的 file 接口操作，体现 Unix 的统一抽象。
