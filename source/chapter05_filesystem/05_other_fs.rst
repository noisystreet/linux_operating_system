...
:strong:`XFS` 由 SGI 为 IRIX 开发，现是 RHEL/Rocky/Alma 的默认文件系统。设计目标是大文件和高吞吐并行 I/O。

核心特点
------------------------

- :strong:`B+ 树` 管理空闲 extent 和 inode，分配与查找效率高
- :strong:`分配组` （allocation groups）将磁盘分区为多个独立单元，减少锁竞争
- :strong:`延迟日志` ：元数据日志，崩溃后快速恢复
- :strong:`在线扩容` ：``xfs_growfs`` 无需卸载即可扩大文件系统
...
:strong:`Btrfs` （B-tree FS）是 COW（写时复制）文件系统：修改数据时写入新块，旧块保留至无引用，天然支持快照与克隆。

核心特点
------------------------

- :strong:`子卷` （subvolume）：逻辑分区，可独立快照
- :strong:`快照` ：只读或读写，瞬间创建，空间按实际变更增长
- :strong:`校验和` ：检测静默数据损坏
- :strong:`压缩` ：透明 zstd/lzo 压缩
- :strong:`RAID 0/1/10/5/6` ：用户态 RAID，RAID5/6 仍有已知风险，生产需谨慎
...
:strong:`tmpfs` 将文件存储在:strong:`页缓存/内存` 中，不落盘（swap 除外）。速度快，适合临时数据；重启或卸载后内容丢失。
...
:strong:`size=`` 限制最大用量，防止耗尽内存。容器中的 ``tmpfs`` 挂载（如 ``/dev/shm``）同样受此约束。
