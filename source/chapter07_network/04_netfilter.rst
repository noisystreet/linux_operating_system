...
数据包在内核协议栈中流动时，需要经过检查、过滤、地址转换等处理。**Netfilter** 是 Linux 内核中的框架，在关键钩子点插入处理逻辑，iptables/nftables 是用户态配置工具。本节介绍 Netfilter 钩子、iptables 基础和 nftables 趋势。
...
**iptables** 是传统的 Netfilter 配置工具，规则组织为 **表** （table）和 **链** （chain）：
...
**filter** 表含 INPUT、OUTPUT、FORWARD 链。默认策略 ``ACCEPT`` 或 ``DROP``，规则按顺序匹配。
...
**连接跟踪** 记录活跃连接状态，使防火墙可识别"已建立连接的回包"并放行，无需为每个回包单独写规则。``RELATED``、``ESTABLISHED`` 状态即基于此。
...
**SNAT** （源 NAT）：内网主机访问外网时，将源 IP 替换为网关公网 IP。**DNAT** （目标 NAT）：将访问网关某端口的流量转发到内网服务器（端口转发）。
...
**nftables** 是 iptables 的继任者，单一框架替代 iptables、ebtables、arptables，语法更统一，内核接口更高效。近年发行版逐步默认 nftables。
...
Netfilter 提供静态规则过滤，**eBPF** 则允许在内核中运行自定义程序，实现更灵活的观测和过滤——下一节介绍 eBPF 基础。
