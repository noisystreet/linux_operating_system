...
每层处理本层首部，向上递交载荷或向下封装。**Netfilter** 钩子在各层插入，供防火墙、NAT 等处理（下节详述）。
...
**图** ：Linux 网络协议栈分层数据流（发送方向）

socket 与 sock
========================

**struct socket** 是 BSD socket 在用户空间的抽象，包含类型（SOCK_STREAM/SOCK_DGRAM）、协议族（AF_INET）、状态等。**struct sock** 是协议无关的传输层端点，TCP 对应 ``struct tcp_sock``，UDP 对应 ``struct udp_sock``。
...
**sk_buff** （socket buffer，简称 skb）是 Linux 表示网络包的核心结构（``include/linux/skbuff.h``）。包含：
...
**NAPI** （New API）：高负载时驱动一次中断处理多个包，减少中断次数。收包入口函数 ``netif_receive_skb()`` 位于 ``net/core/dev.c``；TCP 层入口 ``tcp_v4_rcv()`` 在 ``net/ipv4/tcp_ipv4.c``。
...
2. 触发硬件中断（硬中断上下文，尽量短）
3. 调度 **软中断** ``NET_RX_SOFTIRQ``，在软中断中批量调用 ``netif_receive_skb()``
...
**网络命名空间** 使不同容器拥有独立的网络栈：独立的接口、路由表、iptables 规则。``ip netns`` 可管理命名空间（第 10 章容器部分）。
...
**lo** （127.0.0.1）是本地回环接口，数据不离开本机。**AF_INET** 为 IPv4，**AF_INET6** 为 IPv6，**AF_UNIX** 为同一主机进程间通信（第 3 章 IPC 提及）。
