...
#. 按 ``lab_cgroups`` 手工创建 cgroup 并写入 ``memory.max``，在组内运行 ``stress-ng --vm 1`` 观察 OOM 是否仅杀组内进程。
#. 用 ``lab_namespaces`` 中的 ``unshare -n`` 进入新 netns，在宿主机另一终端配置 veth（``05_container`` 分步清单）。
#. 对比 ``docker inspect`` 中 ``HostConfig.Memory`` 与宿主机 ``/sys/fs/cgroup/.../memory.max`` 数值是否一致。
