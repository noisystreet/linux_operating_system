...
GRUB 加载的 ``vmlinuz`` 并非单一的二进制文件，而是一个 **压缩的内核镜像** ：
...
``start_kernel()`` 运行在 **内核线程上下文** 中，此时还没有任何用户态进程。所有初始化都在内核态完成。
...
1. 创建 **idle 线程** （PID 0）—— 每个 CPU 核心一个，在没有可运行任务时执行
2. 创建 **kernel_init 线程** —— 负责挂载根文件系统和启动 init 进程
...
内核初始化过程中产生的日志写入 **内核环形缓冲区** （kernel ring buffer）。这些日志记录了设备探测、驱动加载、内存分配等关键事件，是诊断启动问题的重要线索。
