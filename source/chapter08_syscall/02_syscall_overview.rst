...
:strong:`系统调用` 是用户程序主动触发的、从用户态切换到内核态的入口。触发后：
...
用户程序:strong:`不能` 直接调用内核函数——内核运行在受保护的地址空间，只能通过 syscall 门进入。
...
每个系统调用有唯一:strong:`系统调用号` 。x86-64 上 ``write`` 为 1，``read`` 为 0，``openat`` 为 257（具体以 ``unistd_64.h`` 为准）。用户将号放入 ``rax``，参数按 ABI 放入指定寄存器，执行 ``syscall`` 指令。
...
glibc 包装层在 syscall 返回负值时，取绝对值写入 ``errno`` 并返回 -1。部分 syscall 在信号中断时返回 ``-EINTR`` ，库函数可能自动重试（如 ``read`` 的部分路径），应用直接调用 ``syscall()`` 时需自行处理。

x86-64 与 aarch64 ABI 对比（常用参数寄存器）：
...
内核模块:strong:`不能` 动态添加系统调用——syscall 表在编译时确定。自定义 syscall 需修改内核源码并重新编译，或通过现有 syscall（如 ``ioctl``）扩展。
