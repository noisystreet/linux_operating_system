...
.. warning::
   ...
   修改内核并加载自定义 syscall 仅适合实验环境。发行版内核通常不保留可插拔的 syscall 槽位，**ioctl** 和 netlink 是更常见的内核-用户扩展方式。

自定义 syscall 补丁示例（仅文档）
====================================

以下展示在主线内核源码中添加 ``hello`` 系统调用的典型步骤，**不纳入本仓库构建** ，仅供实验内核参考。
...
系统调用是用户与内核的边界。下一章讨论 **安全与权限** —— 内核如何决定谁可以执行哪些操作。
